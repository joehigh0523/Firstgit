 ...
 * -# Analysis and Waveform Functions:
 ...
 *   - LP_IQXEL_FastPer_Sweep_Ext();
 *   - LP_IQXEL_MultiCaptureEVMAver();
 *   - LP_IQXEL_MultiCapturePowerAver();
 *   - LP_IQXEL_MeasureSpur();
 *   - LP_Meas_InitSeqBuilder();
 *   - LP_Meas_SetSeqinputParams();
 *   - LP_Meas_BuildAndExecute();
 *   - LP_Meas_CheckSeqStatus();
 *
...
#define MAX_NUM_SEQ_STEPS		128
#define MAX_MODULE_NUM   8
#define VERY_LOW_VSG_POWER_DBM	-110
#define Pi 3.14159
#define MAX_MIMO_TESTER_NUM		4
#ifndef MAX_BUFFER_LEN
#define MAX_BUFFER_LEN 4096 // choose not to increase buffer size. Choose to break commands if commands or results is bigger than this buffer
#endif
...
//! AGC mode which is used to get reference level
enum AGC_MODE
{
    AGC_DISABLE = 0,
    AGC_STAGE1,
...
enum IQTESTER_TYPE
{
	...
	IQTYPE_GIG_IF,                      /*!< IQgig IF tester */
	IQTYPE_GIG_5G,                      /*!< IQgig IF tester */
	IQTYPE_XSTREAM,						/*!< IQxstream tester */
	IQTYPE_XSTREAM_M,			    	/*!< IQxstream-> tester */
	IQTYPE_XEL_M8H,                     /*!< IQxel-M8h tester */
	IQTYPE_XSTREAM_5GX,                 /*!< IQxstream-5GX tester */
	IQTYPE_XEL_M8W7G,
	IQTYPE_XEL_M16W7G,
	IQTYPE_XEL_M2W7G
};
...
typedef struct
{
	// Common settings
	double rfFreqHz; 
	double rfAmplDb[MAX_MPTA_PORTS];
	int port[MAX_MODULE_NUM];
	double extAttenDb;
	...
	double sampleFreqHz; 
	int htMode;
	int nonBlockingState;
	double intervalLength; 
	double measurementLength; 
	//int packetOffsetForAnalysis; 
	//int packetCountForAnalysis;

} SeqSetVsaCaptureParams;
...
enum LP_ANALYSIS_TYPE_ENUM
{
	...
	ANALYSIS_PHASE_SHIFT,                /*! Phase shift analysis*/
	ANALYSIS_NBIOT,
	ANALYSIS_CATM,
	ANALYSIS_80211AX,
...
enum LP_TECHNOLOGY_TYPE_ENUM
{
	...
	TECHNOLOGY_SIGFOX,					   /*!< Sigfox analysis */
	TECHNOLOGY_ZWAVE,                      /*!< ZWAVE analysis */
	TECHNOLOGY_NBIOT,                      /*!< NB-IoT analysis */
	TECHNOLOGY_CATM,                       /*!< CAT-M analysis */
...
#define MAX_MEASURMENT_PER_KEYWORD     20

typedef struct
{
	const char* keyword[MAX_MEASURMENT_PER_KEYWORD][20];
	int attribute; 
	int max_keyword_per_measurement;
	int num_measurement;
   int result_storage_type;  // bitmap, bit_0= per chain, bit_1=per stream, bit_2= per user,bit_3=per RU;   all zeros=no specific
}keyword_table;

#define EACH_CHAIN 0x1
#define EACH_STREAM (0x1 <<1)
#define EACH_USER   (0x1 <<2)
#define EACH_PACKETS   (0x1 <<3)
#define EACH_NO_SPECIFY 0     //for common parameter, if within 11AX , we also need to specify user#, otherwise always user_1
#define EACH_USER_EACH_STREAM  (EACH_STREAM|EACH_USER)
#define EACH_CHAIN_EACH_STREAM  (EACH_STREAM|EACH_CHAIN)

#define IS_EACH_CHAIN(X)	         (bool)((X == EACH_CHAIN) || (X == (EACH_CHAIN|EACH_PACKETS))? TRUE : FALSE)
#define IS_EACH_STREAM(X)	         (bool)((X == EACH_STREAM) || (X == (EACH_STREAM|EACH_PACKETS))? TRUE : FALSE)
#define IS_EACH_USER(X)	             (bool)((X == EACH_USER) || (X == (EACH_USER|EACH_PACKETS))? TRUE : FALSE)
#define IS_EACH_PACKETS(X)           (bool)((X & EACH_PACKETS) ? TRUE : FALSE)
#define IS_EACH_NO_SPECIFY(X)	     (bool)((X == EACH_NO_SPECIFY) || (X == (EACH_NO_SPECIFY|EACH_PACKETS))? TRUE : FALSE)
#define IS_EACH_USER_EACH_STREAM(X)  (bool)((X == EACH_USER_EACH_STREAM) || (X == (EACH_USER_EACH_STREAM|EACH_PACKETS))? TRUE : FALSE)
#define IS_EACH_CHAIN_EACH_STREAM(X)  (bool)((X == EACH_CHAIN_EACH_STREAM) || (X == (EACH_CHAIN_EACH_STREAM|EACH_PACKETS))? TRUE : FALSE)

#define MAX_HE_SIG_B_BITS   128    //maximum 64 bits for 160MHz signal, e.g. 00000010.
#define MAX_RU_ALLOC_SIZE   256    //maximum 74 RUs for BW160 and BW80+80, e.g. (3,1,0,1,1,1)
#define MAX_MAC_ADDR_SIZE   64
#define MAX_NUM_OF_MAC_ADDR 4
#define MAX_AID12_SIZE      12   // 3 bytes for each user and up to 74 users plus ',' between them // YQ Li 3 HEX charactor
#define MAX_11AX_USER_NUM   256  // user number of MU-OFDMA+MU-MIMO case @ BW-160MHz as (8+8)*8+2 (central-26tone)=130
#define MAX_NUM_OF_MPDU     64
#define MAX_FCON_SIZE       10
#define MAX_QOS_SIZE		10
...
typedef struct
{
	int numSegmsToAnalyze;
	int numOfPacketToSkip;
	int numOfPacketToAnalyze;
	char mode[MAX_RESPONSE];
    int enablePhaseCorr;
    ...
    int frequencyCorr;
	int enablePreambleAveraging;
    PACKET_FORMAT packetFormat;
    double prePowStartSec;
    double prePowStopSec;

    int  num_user_triggered;            //for trigger-based PPDU analysis, set to zero if dealing with normal SU or MU packets
    GILTF_AX  GILTF;                    //for trigger-based PPDU analysis
    int  A_factor;                      //for trigger-based PPDU analysis, 
    int  PE_Disambiguity;               //for trigger-based PPDU analysis
    int  NLTF;                          //for trigger-based PPDU analysis, number of HE-LTF
    int  extra_symbol_LDPC;             //for trigger-based PPDU analysis, LDPC extra symbol, use '0' if no LDPC coding    
    int  *MCS_alloc_user;               //for trigger-based PPDU analysis, this is the input value for "CONF:OFDM:MCS:AUS", size=num_user_triggered
    int  *RUindex_alloc_user;           //for trigger-based PPDU analysis, this is the input value for "CONF:OFDM:RUIN:AUS", size=num_user_triggered
    int  *num_SS_user;                  //for trigger-based PPDU analysis, input value for "CONF:OFDM:NSST:AUS" ,size=num_user_triggered
    OFDM_CODING *triggered_coding_user; //for trigger-based PPDU analysis, input value for "CONF:OFDM:COD:AUS", size=num_user_triggered
    int *primary_80MHz_user;            //for trigger-based PPDU analysis, primary (set to 1) or secondary (set to 2) 80MHz for BW-160 or 80+80, use all '1' if not BW-160 nor 80+80; size=num_user_triggered
	int *DCM;                           //for trigger-based PPDU analysis, 1 repsents DCM is enabled, 0 represents DCM is disabled, size=num_user_triggered
} Analysis_11AX_Params;

enum CENTER_26_TONE_RU
{
	DISABLE_ALL,
	PRIMARY_80,
	SECONDARY_80,
...
typedef struct WAVEGEN_OFDM_COMMON
{
	int       numOfUsers;       //number of users
	double    cbw;              //channel bandwidth in Hz
	double    packetPreGap;        //packet Gap in seconds
	double    packetPostGap;        //packet Gap in seconds
	int       numOfExtraAnt;    //Sets the number of extra antennas
    int       isShortGI;        // Set the shourt guard interval for WiFi 11ac/n/a/g/p wave generation.
    int       STBC;             // STBC, 0 to 2 based on technology.
    bool      isBW160Contiguous;    // decide whether BW-160 or BW80_80, only take effect when cbw=160MHz

	SMAP_TYPE spatialMapping;                //Sets the type of Spatial Mapping
	char*     spatialMappingMatrix;          //Sets the spatial mapping matrix ex-  common (#Tx x #Space Time Streams) tones - (#Tx x #Space Time Streams x #Tones)

	/////////////////Waveform degradation settings//////////////////
	double         symbolClockDEGR;//Sets the degradation of symbol clock error in ppm for the waveform.

	/////////////////11ax Setting below////////////////
	GILTF_AX GILTF;            //guard interval and LTF configuration
	int      ELTF;             //Number of Extra HE LTF Symbols. Only valid for 802.11ax packets.
	int      BSSColor;         //Sets the BSS color, as defined in the HE-SIG-A fields
	
	//The below section for 11ax HE MU PPDU signal
	char     sigBcBits[MAX_HE_SIG_B_BITS];     //Sets the HE-SIG-B common bits as defined in 802.11ax, maximum 64 bits for 160MHz signal, e.g. 00000010.
	CENTER_26_TONE_RU CRU;                     //Sets the HE-SIG-B common field bit to enable the center 26-tone RU to be allocated. There are 2 center 26-Tone RUs 
	...
	char     ruAlloc[MAX_RU_ALLOC_SIZE];       //Sets the number of users per RU, separated by comma and enclosed by parenthesis, maximum 74 RUs for BW160 and BW80+80, e.g. (3,1,0,1,1,1)
	char     powerBoostFac[MAX_RU_ALLOC_SIZE]; //Sets the power boost factor (alpha) per RU. 
	int      sigBmcs;                          //Sets the Modulation and Coding Scheme for HE-SIG-B field in WiFi 11ax wave generation.
	PSDU_MAC_HEADER_MPDU MAC_Header_MPDU_State;//Sets whether MAC header settings in a PSDU are general for all MPDUs or individual per MPDU. 
	//The below setting for 11ax HE extended range SU PPDU
	int      bw_eran;          //Sets the bandwidth for HE extended range SU PPDU: 0 for 242-tone RU and 1 for right 106-tone RU within the primary 20MHz.
	
	//The below setting for HE trigger-based PPDU
	bool     codingParmaTFR;      //Sets whether the PSDU coding parameters are based on trigger frame settings. 
	int      legacySigLength;     //Sets the Legacy-SIG Length field for 802.11ax packet generation. only valid when codingParmaTFR is true.

	//The below section for 11ax trigger frame & HE trigger-based PPDU
	int      aFactorTFR;          //Sets the A-factor in packet extension subfield of the Common Info field for purpose of 802.11ax trigger frame generation, value 1-4
	bool     PEDisambTFR;         //Sets the PE-disambiguity bit in packet extension subfield of the Common Info field for purpose of 802.11ax trigger frame generation.
	bool     extranSymLDPCTFR;    //Sets the LDPC extra symbol segment subfield of the Common Info field for purpose of 802.11ax trigger frame generation. 

	//The below section for 11ax trigger frame
	int      numOfUsersTFR;       //Sets the number of users for purpose of 802.11ax trigger frame
	double   cbwTFR;              //Sets the Channel Bandwidth subfield of the Common Info field for purpose of 802.11ax trigger frame 
	int      duration;            //Sets the Duration/ID field for MAC Header for WiFi Packets, input four hex values, e.g. 0x0010
	int      triggeredLengthTFR;  //Sets the length subfield of the Common Info field (12 bits) for purpose of 802.11ax trigger frame generation, input three hex values, e.g. 0x010
	double   APTxPowerTFR;        //Sets the AP Tx Power subfield of the Common Info field for purpose of 802.11ax trigger frame generation
	int      numOfHELTFsymTFR;    //Sets the number of HE_LTF Symbols field in the Common Info field
	int      MUMIMOLTFmodeTFR;    //Sets the MU MIMO LTF mode field in the Common Info field
	int      mPADTFR;             //Sets the padding parameter mPAD for purpose of 802.11ax trigger frame generation.
	bool     csRequiredTFR;       //Sets the CS Required subfield of the Common Info field for purpose of 802.11ax trigger fram generation.
	GILTF_AX giltfTFR;            //Sets the GI and LTF type subfield 
} WAVEGEN_OFDM_COMMON_SETTINGS;

typedef struct WAVEGEN_OFDM_USER
{
    int            numOfSST;                        //number of spatial streams
	int            mcs;                             //MCS index
	char           macAddr[MAX_NUM_OF_MAC_ADDR][MAX_MAC_ADDR_SIZE];	    //MAC address
	OFDM_CODING    coding;                          //OFDM coding
	WIFI_PSDU_TYPE psduType;                        //PSDU Type
	bool           psduCRC;                         //Sets the state of CRC appending
	char           *psduBits;                       //PSDU bits if selected PSDU type is PSDU_USER
	int            stationID;                       //Sets the station ID, as defined in the HE-SIG-B fields
	bool           dcm;             //Sets whether DCM is enabled for WiFi 11ax 
	double         mped;            //Sets the max packet extension duration for all users.

	//The below section for HE trigger-based PPDU 
	int            RUIndex;        //Sets the RU Allocation Index subfield of the User Info field for all users for purpose of 802.11ax trigger frame generation.
    int            freqeuncySegment80MIndex; // Sets the frequency segment for 80+80/160MHz trigger-based PPDU. 0: primary 80MHz, 1: secondary 80MHz.
	double         delayDEGR;      //Sets the time delay degradation for all users. Only applies to 802.11ax trigger-based PPDU.
	double         frequencyDEGR;    //Sets the degradation of frequency error for all users. Only applies to 802.11ax trigger-based PPDU.
	double         powerDEGR;      //Sets the degradation of relative power for all users. Only applies to 802.11ax trigger-based PPDU.
	double         symbolClockDEGR;//Sets the degradation of symbol clock error for all users. Only applies to 802.11ax trigger-based PPDU.

	//The below section for 11ax HE MU PPDU
	ENAB		   mpduQosEnab[MAX_NUM_OF_MPDU]; // Enables the QOS field within the MAC header per MPDU
	char		   qosField[MAX_QOS_SIZE]; // Sets the QOS field in the MAC header
    ENAB           aggregationMPDUEnab; // Enables aggregation-MPDU for 11n, 11ac, and 11ax. Note: AUTO for 11n means OFF. AUTO for 11ac means ON. For 11ax, it is always required ON or AUTO.
	int            numOfMPDU;// Sets the number of MPDUs to be included in the WiFi packet generation. 
	int            mpduPsduLength[MAX_NUM_OF_MPDU]; //Sets the length of WiFi PSDU in octets for packet generation. In case of A-MPDU it specifies the length of MPDU<suffix for the MPDU number>. 
	ENAB           mpduMacAddrEnab[MAX_NUM_OF_MPDU][MAX_NUM_OF_MAC_ADDR]; //Sets whether the address fields (1-4) will be present in MAC header per MPDU. 
	char           mpduMacAddr[MAX_NUM_OF_MPDU][MAX_NUM_OF_MAC_ADDR][MAX_MAC_ADDR_SIZE]; //Sets the user specified MAC Address per MPDU for WiFi Packets. 
	char           mpduFrameControl[MAX_NUM_OF_MPDU][MAX_FCON_SIZE];// Sets the Frame control field for MAC Header per MPDU for WiFi Packets.
	char		   mpduQosField[MAX_NUM_OF_MPDU][MAX_QOS_SIZE]; // Sets the QoS field for MAC Header per MPDU for WiFi Packets.
	ENAB		   mpduSeqEnab[MAX_NUM_OF_MPDU]; // Enables the SEQ field within the MAC header per MPDU
	//char           mpduSeqField[MAX_NUM_OF_MPDU][MAX_SEQ_SIZE]; //Sets the Sequqence Control field for MAC Header per MPDU for WiFi Packets.
    ENAB		   mpduEOFEnab[MAX_NUM_OF_MPDU]; // Enables the EOF field within the A-MPDU delimiter per MPDU

	//The below section for 11ax HE MU PPDU inclurding trigger frame
	int            userAllocTFR ;        //Sets the number of users in trigger frame per PSDU 

	// The below section for  HT Control field
	ENAB           htControlEnable;      //Sets whether the HT Control field will be present in the MAC header.
	// The below section for UL MU response scheduling (UMRS) Control
	int            umrsMcs;              //Sets the MCS subfield for purpose of 802.11ax UL MU response scheduling (UMRS) control generation.
	int            umrsNumOfSymbol;      //Sets the number of data symbols for purpose of 802.11ax UL MU response scheduling (UMRS) control generation.
	double         umrsPow;              //Sets the downlink (DL) transmit (TX) power for purpose of 802.11ax UL MU response scheduling (UMRS) control generation.
	double         umrsRssi;             //Sets the uplink (UL) target RSSI for purpose of 802.11ax UL MU response scheduling (UMRS) control generation.
	int            umrsRuin;             //Sets the RU allocation index for purpose of 802.11ax UL MU response scheduling (UMRS) control generation. 
} WAVEGEN_OFDM_USER_SETTINGS;


typedef struct WAVEGEN_TFR_USER
{
	//The below section for 11ax trigger frame
	int            startingSSTTFR;  //Sets the starting SS in the SS Allocation field in the User Info field
	OFDM_CODING    codingTFR;       //Sets the Coding Type subfield of the User Info field for all users for purpose of 802.11ax trigger frame 
	int            numOfSSTTFR;     //Sets the number of spatial streams for all users for purpose of 802.11ax trigger frame
	int            mcsTFR;          //Sets the MCS subfield of the User Info field for all users for purpose of 802.11ax trigger frame
	int            RUIndexTFR;      //Sets the RU Allocation Index subfield of the User Info field for all users for purpose of 802.11ax trigger frame generation
	bool           dcmTFR;          //Sets the DCM subfield of the User Info field for all users
	char           AID12TFR[MAX_AID12_SIZE];   //Sets the AID12 subfield of the User Info field for all users for purpose of 802.11ax trigger frame generation
	double         targetRSSITFR;    //Sets the RSSI subfield of the User Info field for all users for purpose of 802.11ax trigger frame generation
	int            TIDAggreLimitTFR; //Sets the TID Aggregation Limit subfield in the User Info field of optionally given user for purpose of 802.11ax basic trigger frame generation. It indicates the MPDUs allowed in an A-MPDU carried in the HE TB PPDU and the maximum number of TIDs that can be aggregated by the STA in the A-MPDU
	int            freqSeg80MIndexTFR; // Sets the frequency segment for 80+80/160MHz. 0: primary 80MHz, 1: secondary 80MHz.
} WAVEGEN_TFR_USER_SETTINGS;

typedef struct WAVEGEN_OFDM
{
    WAVEGEN_OFDM_COMMON_SETTINGS commonSettings;
	WAVEGEN_OFDM_USER_SETTINGS *perUserSettings;
	WAVEGEN_TFR_USER_SETTINGS *perTFRUserSettings;
} WAVEGEN_OFDM_SETTINGS;

enum DSSS_FILTER_TYPE
{
	FILTER_RC,
	FILTER_RRC,
	FILTER_GAUSS,
...
typedef struct WAVEGEN_DSSS
{
	double    packetPreGap;                         //packet Gap in seconds
	double    packetPostGap;                        //packet Gap in seconds

	int       numOfExtraAnt;                        //Sets the number of extra antennas for spatial mapping, set to 0 if no spatial mapping is needed
	SMAP_TYPE spatialMapping;                       //Sets the type of Spatial Mapping
	char*     spatialMappingMatrix;                 //Sets the spatial mapping matrix ex-  common (#Tx x #Space Time Streams)

	double         dataRate;                        //the Data Rate with unit Mbps for WiFi 802.11b packet, e.g. 5.5
	char           macAddr[MAX_NUM_OF_MAC_ADDR][MAX_MAC_ADDR_SIZE];	    //MAC address
	WIFI_PSDU_TYPE psduType;                        //PSDU Type
	bool           psduCRC;                         //Sets the state of CRC appending
	char           *psduBits;                       //PSDU bits if selected PSDU type is PSDU_USER
	...
	int             preamble;						//0 for Preamble OFF; 1 for short Preamble; 2 for long Preamble.
	DSSS_FILTER_TYPE filter;                        //Sets type of Transmit filter, default can be set to FILTER_RC
	double          filterBT;                       //Sets the 3-dB bandwidth-symbol time product for transmit filter, default can be set to 0.5
	double          filterLength;                   //Sets filter length for transmit filter, default can be set to 6
    double          filterRollOff;                  //Sets the rolloff factor for transmit filter, default can be set to 0.5
} WAVEGEN_DSSS_SETTINGS;

typedef struct
{
	// Common settings
	int rfPort[MAX_NUM_PORTS]; // PORT array for MIMO configuration (example 3x3 single DUT or APT on RF3A/3B/3C: [4 4 4 0]; example 2up SPT RX on RF1A/1B/1C and RF3A/3B/3C [2 2 2 0; 4 4 4 0])
	int numOfPowerLevels; // Zize of TXpowerLeveldBm or RXpowerLeveldBm array
	double  rfFreqHz; // Frequency of amg subset for the sequence step
	double cableLoss[MAX_NUM_PORTS]; // Cable loss array for MIMO configuration (usage convention is the same as rfPort[] 
	double TXtargetPowerLeveldBm[MAX_NUM_SEQ_STEPS]; // For TX packet or Rx ACK target power list
	...
	// RX settings
	char waveformFileName[MAX_BUFFER_LEN]; // VSG waveform file
	double  RXtargetPowerLeveldBm[MAX_NUM_SEQ_STEPS];  // RX target powers (for any combinations of L,H,S)
	int	    packetCount; // VSG frame count

	// RX AP TBT setting
	double TFRTimeout; // Trigger Frame Timeout in seconds
	double TFRLength; // Trigger Frame Length in seconds
	double ACKLength; // ACK length in seconds
	double ACKDelay; // Time between UL PPDU and AP ACK
	double delay; // Delay between trigger frame and UL PPDU response

	// TX capture settings
	int	captureLengthUs; // VSA capture length
	int	preTriggerTimeUs; // VSA capture pre-trigger time
	int	skipPktCnt; // VSA capture skip packet count
	int	captureCnt; // VSA capture packet count
	int	dataRateType;
	int	bandwidthType; 
	int diversityCDD; // True MIMO vs TX CDD mode
	...
	double prePowStartSec;
	double prePowStopSec;
	double packetDetectionThr; //STA-188
	int  num_user_triggered;            //for trigger-based PPDU analysis, set to zero if dealing with normal SU or MU packets
	GILTF_AX  GILTF;                    //for trigger-based PPDU analysis
	int  A_factor;                      //for trigger-based PPDU analysis, 
	int  PE_Disambiguity;               //for trigger-based PPDU analysis
	...
	int dcRemoveFlag11B;
	int method11B;
	int fixed01DataSequence11B;
	//Wifi Analysis Info - 11n
	char analyzeMimoMode11n[MAX_BUFFER_LEN];
...
// Structure of user info in a waveform file. 
typedef struct tWaveformUserInfo
{
    char macAddr[MAX_NUM_OF_MAC_ADDR][MAX_MAC_ADDR_SIZE];
} WaveformInfoUserInfo;

// Structure of infos in a waveform file. Extensible for customers' requests. Used for LP_GetWaveformInfo function.
typedef struct tWaveformInfo
{
    int numOfUsers;       //number of users

    WaveformInfoUserInfo userInfo[MAX_11AX_USER_NUM];
} WaveformInfo;
 ...
 * \return ERR_OK if the tester has been successfully initialized; otherwise call LP_GetErrorString() for detailed error message.
 */
IQMEASURE_API int       LP_DualHead_Open(int IQxelConnectionType, int tokenID, unsigned int timeOutMS, int sessionNum, unsigned int probeTimeOutMS);
...
IQMEASURE_API int		LP_ReleaseShareControl();
...
//! Saves the waveform I/Q to a IQvsg file (.iqvsg)
/*!
* \param[in] iData float array for signal 1[], signal 2[], signal 3[], ...etc concatenated
* \param[in] qData float array for signal 1[], signal 2[], signal 3[], ...etc concatenated
* \param[in] numOfSignals 1 for regular I/Q data, > 1 for MIMO
* \param[in] length up to 2,147,483,647 samples per each signal
* \param[in] sampleFreqHz sampling rate in Hz
* \param[in] fileName Filename only - could be ending with either .iqvsa or iqvsg
* \param[in] localPath Path for local file generated. Complete path with forward slash '/' is required.
* \param[in] loadFileToTester Set to 1 to first upload to /user, then load to VSA or VSG respectively. Set to 0 to generate local file only.
*
* \return ERR_OK if successful; otherwise call LP_GetErrorString() for detailed error message.
*/
 IQMEASURE_API int LP_SaveIQDataToFile(float *iData, float *qData, int numOfSignals, int length, double sampleFreqHz, const char *fileName, const char *localPath, int loadFileToTester = 1); 
...
//! Generates the OFDM waveform and saves it to the tester or the local PC
/*!
 * \param[in] packetFormat Type of the PPDU to be generated.
 * \param[in] waveGenSettings Configurations to be used for the waveform generation.
 *            commonSettings:
 *                - numOfUsers: number of users
 *                - cbw: channel bandwidth in Hz
 *                - packetGap: packet Gap in seconds
 *                - GILTF: guard interval and LTF configuration
 *                - sigBcBits: Sets the HE-SIG-B common bits as defined in 802.11ax, maximum 64 bits for 160MHz signal, e.g. 00000010.
 *                - ruAlloc: Sets the number of users per RU, maximum 74 RUs for BW160 and BW80+80, e.g. (3,1,0,1,1,1)
 *                - duration: Sets the Duration/ID field for MAC Header for WiFi Packets, input four hex values, e.g. 0x0010
 *                - aFactor: Sets the A-factor in packet extension subfield of the Common Info field for purpose of 802.11ax trigger frame generation, value 1-4
 *                - triggeredLength: Sets the length subfield of the Common Info field (12 bits) for purpose of 802.11ax trigger frame generation, input three hex values, e.g. 0x010
 *                - PEDisamb: Sets the PE-disambiguity bit in packet extension subfield of the Common Info field for purpose of 802.11ax trigger frame generation.
 *                - APTxPower: Sets the AP Tx Power subfield of the Common Info field for purpose of 802.11ax trigger frame generation.
 *            perUserSettings:
 *                - numOfSST: number of spatial streams
 *                - mcs: MCS index
 *                - macAddr: MAC addres
 *                - coding: Coding type
 *                - psduLength: PSDU length in bytes
 *                - psduType: PSDU Type
 *                - psduCRC: Sets the state of CRC appending
 *                - psduBits: PSDU bits if selected PSDU type is PSDU_USER
 *                - AID12: Sets the AID12 subfield of the User Info field for all users for purpose of 802.11ax trigger frame generation
 *                - targetRSSI: Sets the RSSI subfield of the User Info field for all users for purpose of 802.11ax trigger frame generation
 *                - RUIndex: Sets the RU Allocation Index subfield of the User Info field for all users for purpose of 802.11ax trigger frame generation
 * \param[in] includeTRFrame Whether to include the trigger frame in the PSDU.
 * \param[in] fileName Generated waveform file name.
 * \param[in] saveToLocal By default the waveform will be saved on the tester /user folder, if enabled this, the waveform will be saved to local PC too.
 * \param[in] localFilePath The location on the local PC to save the generated waveform.
 *
 * \return ERR_OK if successful; otherwise call LP_GetErrorString() for detailed error message.
 */
IQMEASURE_API int       LP_WaveGen_OFDM(WIFI_STAN wifiStandard, PACKET_FORMAT packetFormat, WAVEGEN_OFDM_SETTINGS *waveGenSettings, bool includeTRFrame, char *fileName, bool saveToLocal, char *localFilePath = NULL, bool autoSamplingRate = 1, double samplingRate = 160000000);

//! Generates the DSSS waveform and saves it to the tester or the local PC
/*!
 * \param[in] fileName Generated waveform file name.
 * \param[in] saveToLocal By default the waveform will be saved on the tester /user folder, if enabled this, the waveform will be saved to local PC too.
 * \param[in] localFilePath The location on the local PC to save the generated waveform.
 ...
 * \return ERR_OK if successful; otherwise call LP_GetErrorString() for detailed error message.
 */
IQMEASURE_API int       LP_WaveGen_DSSS(WAVEGEN_DSSS_SETTINGS *waveGenSettings, char *fileName, bool saveToLocal, char *localFilePath = NULL, bool autoSamplingRate = 1, double samplingRate = 160000000);
...
/*!
 * \param[in] modFileName Waveform names to be played; seperate in comma;
 * \param[in] Next Specify the 'next' wave segment to be played following the specified wave segment in the wave list;
 * \param[in] Repeatplay Specify how many times each waveform is played;
 * \param[in] NumberofWaveforms Indicate number of waveforms to be played; This parameter is only effectvie when there is only one waveform name in the first parameter. 
 *	It will then add suffix to the waveforms in the format of "modFileName_1, modFileName_2....."; If the first parameter contains multiple waveforms, this parameter is ignored.
 * \param[in] loadInternalWaveform Choose to load modFile from internal tester (1) or to upload it from PC (0). The default is (0). This option is only valid with IQxel tester otherwise it is ignored.
 ...
 * \return ERR_OK if successful; otherwise call LP_GetErrorString() for detailed error message.
 */
IQMEASURE_API int		LP_SetVsgModulation_MultiWavesExt(char *modFileName, int Next[], int Repeatplay[], int NumberofWaveforms, int loadInternalWaveform = 0);
 ...
 * \remark For VSA to work optimally with the input signal, set rfAmplDb to the peak power of the input signal.
 */
IQMEASURE_API int       LP_SetVsa(double rfFreqHz, double rfAmplDb, int port, double extAttenDb = 0,
                                  double triggerLevelDb = -25, double triggerPreTime = 10e-6,
								  double dFreqShiftHz = 0.0);

IQMEASURE_API int		LP_SetVsaSEQ(SeqSetVsaCaptureParams *seqVsaCaptureSetting);

IQMEASURE_API int		LP_SetVsaPhaseShiftCapture(double rfFreqHz, double rfAmplDb, int port, double SamplingTime, int triggerTimeout, bool AutoRlevEnable = false);
...
//! Select VSA for BT MIMO
/*!
 * \pass down VSA signal number
 * \return ERR_OK if no errors; otherwise call LP_GetErrorString() for detailed error message.
 */
IQMEASURE_API int       LP_SelectBTSignal(int signalNumber);
 ...
 * \param[in] waveSegment specify the wave segment
 *
 * \return ERR_OK if number of frames accepted; otherwise call LP_GetErrorString() for detailed error message.
 */
IQMEASURE_API int		LP_SetFrameCntMultiWave(int frameCnt, int waveSegment = 1);
...
//! Turn ON/OFF the VSG RF for dynamic module numbers
/*!
 * \param[in] rxEnabled as an array to turn on the RF of corresponding RX path; 1 to turn on RF; 0 to turn off RF
 * \param[in] Max_Num_rxEnabled represents the largest number of activated modules
 * \return ERR_OK if the first VSG RF is turned on or off; otherwise call LP_GetErrorString() for detailed error message.
 */
IQMEASURE_API int		LP_EnableVsgRFMimoNxN(int* rxEnabled, int Max_Num_rxEnabled);
...
//! Retrieves the captured I/Q raw data from VSA
/*!
 * \param[in] vsaNum Number of VSA (0-3). Currently not used.
 * \param[in] resampleFreqHz resample frequency, for example, 200kHz or 10MHz.
 * \param[out] bufferReal Returns I samples
 * \param[out] bufferImag Returns Q samples
 * \param[in] bufferLength Indicates the length of bufferReal and bufferImag (in bytes)
 *
 * \return ERR_OK if no erros; otherwise call LP_GetErrorString() for detailed error message.
 */
IQMEASURE_API int		LP_GetResampledData(int vsaNum, double resampleFreqHz, double startSec, double stopSec, float* bufferReal, float* bufferImag, int* bufferLength);
...
//! Turn SendtoPeer falg in SCPI driver
/*!
 * \param[in] swith: turn on or off the sendToPeer flag 
 *
 * \return ERR_OK if no errors; otherwise call LP_GetErrorString() for detailed error message.
 */
IQMEASURE_API int		LP_SetPeerSendFlag(bool flag);

//! Connect to Peer tester
/*!
 * \param[in] ip: IP address of peer tester 
 *
 * \return ERR_OK if no errors; otherwise call LP_GetErrorString() for detailed error message.
 */
IQMEASURE_API int		LP_ConnPeerTester(char* ip, int mode = 0 );
...
//! Install TFC library  to IQxel tester
/*!
 * \param[in] fileNames The names of the files needed for the TFC library, separated by comma ","
 * \param[in] libraryName The name of the major TFC library
 * \return ERR_OK if no errors;
*/
IQMEASURE_API int		LP_InstallTFCLibrary(char *fileNames, char *libraryFileName); 
...
/*!
 * \return ERR_OK if the phase shift analysis succeeded; otherwise call LP_GetErrorString() for detailed error message.
 */
IQMEASURE_API  int     LP_AnalyzePhaseShift(double analysisLength_us);
 ...
 * \return ERR_OK if no errors; otherwise call LP_GetErrorString() for detailed error message.
 */
IQMEASURE_API int		LP_AnalyzeZigbee(char *spec_duration="PACKET");

//! Perform Sigfox Analysis on current capture
/*!
 * \param[in] startSec, where in the capture the calculation should begin, in seconds. Typically it's 0.
 * \param[in] stopSec, where in the capture the calculation should finish, in seconds. 
 * \param[in] bitrate, Sigfox bit rate of TX signal in bps, from -99e36 to 99e36, default 600.
 * \param[in] rczIndex, Sigfox radio configuration zone index, -2147483648 to 2147483647
 * \return ERR_OK if no errors; otherwise call LP_GetErrorString() for detailed error message.
 */
IQMEASURE_API int		LP_AnalyzeSigfox(double startSec, double stopSec, double bitrate = 600.0, int rczIndex = 0);
...
* \return ERR_OK if no errors; otherwise call LP_GetErrorString() for detailed error message.
*/
IQMEASURE_API int		LP_AnalyzeDECT( char *analysis_type = "ALL", int packetCountForAnalysis = 2);
...
//! Perform Power Analysis on GPRF signals
/*!
 * \param[in] interval_length, Sets the interval length for power, phase and spectrum measurements, Default Value: 1e03, Units: S.
 * \param[in] measurement_length, Sets the measurement length starting at beginning of interval for power, phase and spectrum measurements. Default Value: 1e03, Units: S.
 * \param[in] startOffset. Offset in seconds into the capture data for start of calculation: 0~12, Units: S.
 * \param[in] length, Length of calculation in intervals, Default Value: 1, Units: None.
 * \param[in] segment_index, current capture segment in the current active instrument channel, Default Value 1, Units: None.
 */
IQMEASURE_API int       LP_AnalyzeGPRFPower(double interval_length = 1e-3, double measurement_length = 1e-3, double startOffset = 0.0, int length = 1, int segment_index = 1);
...
//! Get a scalar measurement result
/*!
 * \param[in] measurement The measurement name.  
 * \param[in] resultIndex The index of the measurement, specified in the RESULT_INDEX structure. The indecies start from 1. 
 *
 * \return The value of the measurement.  -99999.99 (a special defined negative value) will be returned if no measurement available
 */
IQMEASURE_API double	LP_GetScalarResult(char *measurement, RESULT_INDEX resultIndex);
...
/*!
 * \param[in] measurement The measurement name. 
 * \param[in] resultIndex The indecies of the measurement as specified in the structure. 
 * \param[out] buffer Returns the vector result
 * \param[out] bufferLength Indicates the length of buffer (in Bytes)
 *
 * \return The number of elements in bufferReal or/and bufferImag
 */
IQMEASURE_API int		LP_GetVectorResult(char *measurement, RESULT_INDEX resultIndex, double buffer[], int bufferLength);
...
*         - 0:Frequency correction AUTO. (=LTF for 11ac)
*         - 1:Frequency correction on short legacy training fields
*         - 2:Frequency correction on long legacy training fields (Default)
...
/*!
 * \param[in] input11adParams Analysis_11AD_Params struct instructions are as follows:
 *    - numSegmsToAnalyze indicates segment number to be analyzed:
 *         - 1 (Default)
 *    - numberOfPacketToSkip indicates number of packets to be skipped for analysis
 *         - 0 (Default)
 *    - numberOfPacketToAnalyze indicates number of packets to be analyzed
 *         - 1 (Default)
 *    - freqCorr Sets the frequency correction type for packet analysis in the current active instrument channel:
 *         - 0: OFF
 *         - 1: FULL (Default)
 *    - SymClockCorr Sets the symbol clock correction type for packet analysis in the current active instrument channel:
 *         - 0: OFF
 *         - 1: FERR (According to frequency error) (Default)
 *    - phaseTracking Sets the phase tracking type for packet analysis in the current active instrument channel:
 *         - 0: OFF
 *         - 1: BLOCK (Default)
 *    - dcCorr Sets the phase tracking type for packet analysis in the current active instrument channel:
 *         - 0: OFF
 *         - 1: BLOCK (Default)
 *    - decodePSDU Decode PSDU:
 *         - 0: Disable (Default)
 *         - 1: Enable
 *    - prePowStartSec indicates the start point of preamble for power measurement result "powerPreambleDbm"(IQxel
 * Only):
 *         - 8.8e-6: 8.8 micro second (Default)
 *    - prePowStopSec indicates the end point of preamble for power measurement result "powerPreambleDbm" (IQxel
 * Only):
 *         - 15.2e-6: 15.2 micro second (Default)
 *
 *
 * \return ERR_OK if the 802.11ad analysis succeeded; otherwise call LP_GetErrorString() for detailed error message.
 */
IQMEASURE_API int       LP_Analyze80211ad(Analysis_11AD_Params input11adParams);
...
//! Fast per rx tests(RX ACK) using IQxel;  It supports IQxel 80, IQxel160 Rout1 and Rout2, IQxel-M/MW series.
/*!
 * \param[in] Port  The RF port used 
 *				   2 - Left RF port
 *				   3 - Right RF port
 ...
 * \Param[in] PacketDelayTime Time to wait after receiving Ack to send next packet
*/
IQMEASURE_API int LP_IQXEL_FastPer_Sweep(unsigned int Port,  int FreqMHz, unsigned int VsgTransmittPtkCnt, double VsgPowerStartLevel, 
											   double VsgPowerStopLevel, double PowerStep, char *pcWaveFileName, unsigned int *AckNumReturn, unsigned int *AckArraysize, int PacketDelayTime=10);

//! Fast per rx tests(RX ACK) using IQxel;  It supports IQxel 80, IQxel160 Rout1 and Rout2, IQxel-M/MW series. Enhance LP_IQXEL_FastPer_Sweep() by enabling multiple RF ports for IQxel-M/MW series testers, and enabling double frequency settings.
/*!
 * \param[in] Port  The RF ports used 
 *				- =2: RF port 1 (RF1)
 *				- =3: RF port 2 (RF2)
 *				- =4: RF port 3 (RF3)
 *				- =5: RF port 4 (RF4)
 ...
 * \Param[in] PacketDelayTime Time to wait after receiving Ack to send next packet
*/
IQMEASURE_API int LP_IQXEL_FastPer_Sweep_Ext(unsigned int Port[], double FreqMHz, unsigned int VsgTransmittPtkCnt, double VsgPowerStartLevel, 
											   double VsgPowerStopLevel, double PowerStep, char *pcWaveFileName, unsigned int *AckNumReturn, unsigned int *AckArraysize, int PacketDelayTime=10);
...
//!Set CCK analysis frequency offset, for 20in40 CCK analysis. The calling sequency could be:
//!  LP_Set11bAnalysisFreqencyOffset(-10e6); //-10e6 for left side signal, and 10e6 for right side signal.
//!  LP_Analyze80211b();
//!  LP_Set11bAnalysisFreqencyOffset(0e6); // Restore to 0.
/*!
 * \param[in] frequencyOffsetHz. Shifted frequency offset when analyzing 80211b signal. 
 *
 * \return ERR_OK if successful; otherwise call LP_GetErrorString() for detailed error message.
 */
IQMEASURE_API int LP_Set11bAnalysisFreqencyOffset(double frequencyOffsetHz);

//!Get waveform info from a external or internal waveform file.
/*!
 * \param[in] modFileName. IQVSG file path in local disk or tester. Note: function should be only for non-excrypted IQVSG file.
 * \param[out] outputInfo. Waveform infomation retrived from IQVSG file.
 * \param[in] loadInternalWaveform. Choose to load modFileName from internal tester (1) or to upload it from PC (0). The default is (0).
 * \return ERR_OK if successful; otherwise call LP_GetErrorString() for detailed error message.
 */
IQMEASURE_API int LP_GetWaveformInfo(char *modFileName, WaveformInfo *outputInfo, int loadInternalWaveform = 0);
...
/*!
 *\param[in] crosstalkMethod, default = 1 new universal crosstalk method, 0 old crosstalk checking method 
 * \return 0 if successful.
 */
IQMEASURE_API int LP_Async_InitTxRxHandler(int crosstalkMethod = 1);
 ...
 * \remark modeHT40 only needs to set to 1 if the HT40 mask (120MHz) analysis is desired.  For LP_Analyze80211n(), this flag can be set to 0.
 */
IQMEASURE_API int LP_Segms_VsaDataCapture(int numSegms, double samplingTimeSecs, int triggerType = 6,
                                          double sampleFreqHz = 160e6, int ht40Mode = OFF,
										  IQMEASURE_CAPTURE_NONBLOCKING_STATES nonBlockingState = BLOCKING, bool AutoRlevEnable = false);


//! Perform multiple VSA data captures on different frequencies and stores data to multiple segments.
/*!
 * \param[in] chanList: List of frequencies to capture data on.
 * \param[in] numChans: Number of frequencies passed in. Corresponds to the number of segment captures taken.
 * \param[in] samplingTimeSecs: Capture time in seconds
 * \param[in] triggerType Trigger type used for capturing.  Valid options are:
 *      - 1 = Free-run
 *      - 2 = External trigger
 ...
 * \return ERR_OK if the data capture is successful; otherwise call LP_GetErrorString() for detailed error message.
 */
IQMEASURE_API int LP_Segms_VsaDataCapture_MultiChan(double *chanList, int numChans, double samplingTimeSecs, int triggerType, double sampleFreqHz);



//! Perform multiple VSA data capture and store data to multiple segments. The functions calling sequence is
// LP_Segms_VsaDataCapture --> LP_Segms_Analyze80211ag --> LP_Segms_GetScalarMeasurement
 ...
 *flag can be set to 0.
 */
IQMEASURE_API int LP_Segms_VsaDataCaptureSeq(int numSegms, double samplingTimeSecs, int triggerType = 6,
                                          double sampleFreqHz = 160e6, int ht40Mode = OFF,
                                          IQMEASURE_CAPTURE_NONBLOCKING_STATES nonBlockingState = BLOCKING, double vsaTimeoutMs = 200.0);
 ...
 *         - 2: greenfield format
 * \param[in] frequency Correction options 
 *         - 0 = Frequency correction AUTO. (=LTF for 11n)
 *         - 2 = frequency correction on short and long legacy training fields (default)
 *         - 3 = frequency correction based on full packet
 *		   - 4 = frequency correction on signal fields (legacy and HT) in addition to short and long training fields
 * \param[in] prePowStartSec indicates the start point of preamble for power measurement result "powerPreambleDbm" (IQxel Only):
...
//! Perform 802.11 ax Analysis on current capture
/*!
* \param[in] numSegmsToAnalyze number of segements to analyze
* \param[in] numOfPacketToSkip number of packets to be skipped for analysis in the catpure
* \param[in] numOfPacketToAnalyze number of packets to be analyzed for average EVM
* \param[in] mode Valid options are:
*         - "nxn": for true 802.11ax capture
* \param[in] enablePhaseCorr Phase Correction Mode:
*         - 0: Disable
*         - 1: Enable (Default)
* \param[in] enableSymTimingCorr Symbol Timing Correction:
...
*         - 1: Enable
* \param[in] frequencyCorr Frequency Correlation:
*         - 0:Frequency correction AUTO. (=Whole packet for 11ax)
*         - 1:Frequency correction on short legacy training fields
*         - 2:Frequency correction on long legacy training fields
*         - 3:Frequency correction based on whole packet
*         - 4:Frequency correction on signal fields (legacy and HT) in addition to long training fields
* \param[in] enablePreambleAveraging Preamble Averaging:
*         - 0: Disable
*         - 1: Enable
* \param[in] packetFormat indicates format of the 80211AC MIMO packet:
*         - AUTO: for auto detection
*         - HE_SU: for 11AX HE-SU
*         - HE_MU: for 11AX HE-MU
*         - HE_TR: for 11AX HE-TRG
* \param[in] prePowStartSec indicates the start point of preamble for power measurement result "powerPreambleDbm"(IQxel Only):
*         - 8.8e-6: 8.8 micro second recommanded value
* \param[in] prePowStopSec indicates the end point of preamble for power measurement result "powerPreambleDbm" (IQxel Only):
*         - 15.2e-6: 15.2 micro second recommanded value
* \param[in] num_user_triggered for trigger-based PPDU analysis, set to zero if dealing with normal SU or MU packets
* \param[in] GILTF for trigger-based PPDU analysis
* \param[in] A_factor for trigger-based PPDU analysis
* \param[in] MCS_alloc_user for trigger-based PPDU analysis, the MCS rate for each user, its size should be num_user_triggered
* \param[in] RUindex_alloc_user for trigger-based PPDU analysis, the RU index for each user, its size should be num_user_triggered
*
*
* \return ERR_OK if the 802.11ax analysis succeeded; otherwise call LP_GetErrorString() for detailed error message.
*/
IQMEASURE_API int LP_Analyze80211ax(Analysis_11AX_Params * inputParams);

//! Set 802.11 ax Analysis parameter for sequence based capture
/*!
* \param[in] numSegmsToAnalyze number of segements to analyze
* \param[in] mode Valid options are:
*         - "nxn": for true 802.11ax capture
...
* \return ERR_OK if the 802.11n analysis succeeded; otherwise call LP_GetErrorString() for detailed error message.
*/
IQMEASURE_API int LP_SEQ_Analyze80211ax(Analysis_11AX_Params *inputParams);

//! Set 802.11 ac Analysis parameter for sequence based capture
/*!
* \param[in] mode Valid options are:
*         - "nxn": for true MIMO capture
*         - "evms": for EVM per signal analysis
*         - "sequential_mimo": for a switched capture
* \param[in] enablePhaseCorr Phase Correction Mode:
*         - 0: Disable
...
*         - 3:Frequency correction based on whole packet
*         - 4:Frequency correction on signal fields (legacy and HT) in addition to long training fields
* \param[in] referenceFile reference file name:
*         
* \param[in] packetFormat indicates format of the 80211AC MIMO packet:
*         - 0: auto detect							
*		  - 1: mixed format (11n)					
...
*
*
* \return ERR_OK if the 802.11ac analysis succeeded; otherwise call LP_GetErrorString() for detailed error message.
*/
IQMEASURE_API int LP_SEQ_Analyze80211ac(char *mode,
										int enablePhaseCorr            = 1,
										int enableSymTimingCorr        = 1,
										int enableAmplitudeTracking    = 0,
										int decodePSDU                 = 0,
...
//! Set 802.11n Analysis parameter for sequence based capture
/*!
* \param[in] mode Valid options are:
*         - "nxn": for true MIMO capture
*         - "evms": for EVM per signal analysis
...
* \return ERR_OK if the 802.11n analysis succeeded; otherwise call LP_GetErrorString() for detailed error message.
*/
IQMEASURE_API int LP_SEQ_Analyze80211n( char *type,
										char *mode,
										int enablePhaseCorr            = 1,
										int enableSymTimingCorr        = 1,
										int enableAmplitudeTracking    = 0,
										...
										double prePowStartSec     = 8.8e-6,
										double prePowStopSec     = 15.2e-6);
//! Set 802.11b Analysis parameter for sequence based capture
/*!
* \param[in] ph_corr_mode Valid options are:
*         - 1: Phase correction off
*         - 2: Symbol-by-Symbol correction
*         - 3: Moving average correction
* \param[in] enableFullPacketChannelEst Channel Estimate on full packet:
*         - 0: Disable (Default)
*         - 1: Enable
* \param[in] sym_tim_corr Symbol Timing Correction:
*         - 0: Disable
*         - 1: Enable (Default)
* \param[in] freq_sync frequency synchronize:
*         - 1: Short trainning symbol
*         - 2: Long trainning symbol
*         - 3: Full data packet
* \param[in] ampl_track Amplitude tracking:
*         - 1: Off
*         - 2: On
* \param[in] prePowStartSec indicates the start point of preamble for power measurement result "powerPreambleDbm"(IQxel Only):
*         - 8.8e-6: 8.8 micro second recommanded value
...
* \return ERR_OK if the 802.11ac analysis succeeded; otherwise call LP_GetErrorString() for detailed error message.
*/
IQMEASURE_API int LP_SEQ_Analyze80211ag(int ph_corr_mode           = 2, 
										int ch_estimate            = 1, 
										int sym_tim_corr           = 2, 
										int freq_sync              = 2, 
										int ampl_track             = 1, 
...
//! Set 802.11b Analysis parameter for sequence based capture
/*!
* \param[in] eq_taps equalizer taps:
*         - Opions are 1,5,7,9
* \param[in] DCremove11b_flag:
*         - 0: Disable (Default)
*         - 1: Enable
* \param[in] method_11b Timing Correction:
*         - 1: Use EVM method standard 1999
*         - 2: Use 11b RMS error vector
*         - 3: Use EVM method standard 2007
*         - 3: Use EVM method standard 2016
* \param[in] fixed_01_data_seq frequency synchronize:
*         - 0: Disable
*         - 1: Enable
* \param[in] prePowStartSec indicates the start point of preamble for power measurement result "powerPreambleDbm"(IQxel Only):
*         - 8.8e-6: 8.8 micro second recommanded value
...
*
*
* \return ERR_OK if the 802.11b analysis succeeded; otherwise call LP_GetErrorString() for detailed error message.
*/
IQMEASURE_API int LP_SEQ_Analyze80211b( int eq_taps                = 1, 
										int DCremove11b_flag       = 0, 
										int method_11b             = 1, 
										int fixed_01_data_seq      = 0, 
										double prePowStartSec = 8.8e-6,
										double prePowStopSec = 15.2e-6);
...
//! Perform ramp Analysis on current capture
/*!
 * \param[in] numSegmsToAnalyze number of segements to analyze

 * \return ERR_OK if no errors; otherwise call LP_GetErrorString() for detailed error message.
 */
IQMEASURE_API int LP_Segms_AnalyzePowerRamp80211b(int numSegmsToAnalyze);
...
//! Perform CW power and Freq error Analysis on current capture, find section with maximum power and calcualte frequency error from that section

/*!
 * \param[in] intervalLength set the interval length of the capture. Default to 0.
 * \param[in] captureLength sets the capture length starting at the begining of the interval. Default to 0.
 *
 * \return ERR_OK if no errors; otherwise call LP_GetErrorString() for detailed error message.
 */
IQMEASURE_API int LP_Segms_AnalyzeCWMaxPowSegm(double intervalLength = 0.0, double captureLength = 0.0);
 ...
 * \return ERR_OK if no errors; otherwise call LP_GetErrorString() for detailed error message.
 */
IQMEASURE_API int LP_Segms_AnalyzeZigbee(int numSegmsToAnalyze, char* spec_duration = "PACKET");
 ...
 * \param[in] optionVsaVsg - 1:Apply the RF correction to VSA, 2: Apply the RF correction to VSG 3: Apply the RF
 *Correction to both VSA/VSG.
  * \param[in] bankId - specfiy the bank ID for IQxstream-M.
 * \return ERR_OK if successful;
 */
IQMEASURE_API int LP_ApplyRFCorrectionExt(char *pathLossTableName, int port, int moduleIndex,
                                       int optionVsaVsg, int bankId = 1);


/*! Query current RF correction for the specified port and frequency.
 * \param[in] port The port to which the VSA/VSG module connects, with the following options:
 *				- =1: OFF
 *				- =2: RF port 1 (RF1)
 *				- =3: RF port 2 (RF2)
 ...
 *				- =3: VSA/VSG21
 *				- =4: VSA/VSG22
 * \param[in]  rfFreqHz - Port Correction frequency
 * \param[in] optionVsaVsg - 1:Apply the RF correction to VSA, 2: Apply the RF correction to VSG
 * \param[out] loss, the port correction for the specified port and frequency.
 * \return ERR_OK if successful;
 */
IQMEASURE_API int LP_QueryRFCorrection(int port, int moduleIndex, double rfFreqHz, int optionVsaVsg, double *loss);
 ...
 * \param[in] optionVsaVsg - 1:Apply the RF correction to VSA, 2: Apply the RF correction to VSG 3: Apply the RF
 *Correction to both VSA/VSG.
 * \param[in] turn_on - 1:Enable the RF correction to VSA/VSG, 0: Disable the RF correction to VSA/VSG
 * \return ERR_OK if successful;
 */
IQMEASURE_API int LP_SwitchRFCorrection( int port, int moduleIndex,
                                       int optionVsaVsg, bool turn_on);
...
//! Get internal waveform directory.
//! The internal waveform directory stores the waveform files which are 
//!   1. uploaded by using LP_SetVsgModulation with loadInternalWaveform = 0.
//!   2. generated by using LP_WaveGen_OFDM()/ LP_WaveGen_DSSS() functions.
//! Note: Function is valid only if tester is connected.
/*!
 * \param[out] waveformDirStr. String buffer to store internal waveform directory. The directory string does NOT end with slash('/' or '\\').
 * \param[out] strBufSize. The declared string buffer size of waveformDirStr. Suggested string buffer size is 256, ie. char waveformDirStr[256].
 * \return ERR_OK if no errors; Return ERR_BUFFER_OVERFLOW if strBufSize is not enough. Return ERR_INVALID_PARAM_SETTING if waveformDirStr is NULL.
*/
IQMEASURE_API int LP_GetInternalWaveformDirectory(char *waveformDirStr, const size_t strBufSize);
//**************************************************************************//
//                              NFC START                                   //
//**************************************************************************//
 ...
 * \return ERR_OK if no errors; otherwise call LP_GetErrorString() for detailed error message.
 */
IQMEASURE_API int LP_IQLINK_SetRxMapi(IQLINK_BAND apBand, IQLINK_TECH_MODE apMode, IQLINK_BANDWIDTH apBandwidth, int apChannel, char* dataRate, int apNumStream, char* mapiAssocMode, char* dutControlIP, int rf1, int rf2, int rf3,
 ...
 * \return ERR_OK if no errors; otherwise call LP_GetErrorString() for detailed error message.
 */
IQMEASURE_API int LP_IQLINK_SetTxMapiSweep(IQLINK_BAND apBand, IQLINK_TECH_MODE apMode, IQLINK_BANDWIDTH apBandwidth, int apChannel, char* dataRate, char* expectedDutRate, int apNumStream, char* mapiAssocMode, char* dutControlIP, int rf1, int rf2, int rf3,
									  double sweepStart, double sweepStop, double sweepStep, int autoSense, int txPacketCountThreshold = 100, double txpTime = 5.0, double txpInterval = 0.001);
...
//                            Temp Comp START                           //
//**********************************************************************//
//Get the diff cal temperature for Factory calibration on specified VSA module 
/*!
 * \param[in] module_index, which indicates the module index, for example 1 -> module A, 2 -> module B.
 * \param[out] current_temp, return the current diff temperature in degree celsius.
 * \param[out] prev_temp, return the previous diff temperature in degree celsius, saved in variable "TEMP_DIFF_VSA".

 * \return ERR_OK if no errors; otherwise call LP_GetErrorString() for detailed error message.
*/
IQMEASURE_API int LP_GetVsaCompDiffer(unsigned int module_index, double* current_temp, double* prev_temp);

//Get the diff cal temperature for Factory calibration on specified VSG module 
/*!
 * \param[in] module_index, which indicates the module index, for example 1 -> module A, 2 -> module B.
 * \param[out] current_temp, return the current diff temperature in degree celsius.
 * \param[out] prev_temp, return the previous diff temperature in degree celsius, saved in variable "TEMP_DIFF_VSG".

 * \return ERR_OK if no errors; otherwise call LP_GetErrorString() for detailed error message.
*/
IQMEASURE_API int LP_GetVsgCompDiffer(unsigned int module_index, double* current_temp, double* prev_temp);

//Get the temperature compensation state of the specified VSA module. 
/*!
 * \param[in] module_index, which indicates the module index, for example 1 -> module A, 2 -> module B.
 * \param[out] status, return the current state ON/OFF.

 * \return ERR_OK if no errors; otherwise call LP_GetErrorString() for detailed error message.
*/
IQMEASURE_API int LP_TempComp_CheckVsaStatus(unsigned int module_index, unsigned int* status);

//Get the temperature compensation state of the specified VSG module. 
/*!
 * \param[in] module_index, which indicates the module index, for example 1 -> module A, 2 -> module B.
 * \param[out] status, return the current state ON/OFF.
 ...
 * \return ERR_OK if no errors; otherwise call LP_GetErrorString() for detailed error message.
*/
IQMEASURE_API int LP_TempComp_CheckVsgStatus(unsigned int module_index, unsigned int* status);

//Run the temperature compensation on the specified VSA module. 
/*!
 * \param[in] module_index, which indicates the module index, for example 1 -> module A, 2 -> module B.

 * \return ERR_OK if no errors; otherwise call LP_GetErrorString() for detailed error message.
*/
IQMEASURE_API int LP_TempComp_TriggerVsa(unsigned int module_index);

//Run the temperature compensation on the specified VSG module. 
/*!
 * \param[in] module_index, which indicates the module index, for example 1 -> module A, 2 -> module B.

 * \return ERR_OK if no errors; otherwise call LP_GetErrorString() for detailed error message.
*/
IQMEASURE_API int LP_TempComp_TriggerVsg(unsigned int module_index);
...
//! Sets up VSG
/*!
 * \param[in] band, Sets the band in the current active instrument channel.
 * \param[in] channel, Sets the EARFCN for LTE uplink analysis.
 * \param[in] rfPowerLeveldBm, The output power level of VSG (dBm)
 * \param[in] port, The port to which the VSG connects, with the following options:
 *				- =1: OFF
 *				- =2: RF port 1 (RF1)
 ...
 *				- =4: RF port 3 (RF3)
 *				- =5: RF port 4 (RF4)
 * \param[out] currentFreqDL, the converted frequency value for downlink VSG.
 * \param[out] currentFreqUL, the converted frequency value for uplink VSA if autoConvert is true.
 * \param[in] autoConvert, if enabled, the uplink frequency would converted and set to VSA. 
 * \param[in] rfFreqHz The center frequency of VSG (Hz). Set real center frequency, if set to non-zero value, the frequency value would overwrite the setting for BAND, CHANNEL.
 * \return ERR_OK if no errors; otherwise call LP_GetErrorString() for detailed error message.
 */
IQMEASURE_API int       LP_SetVsgNBIoT(int band, double channel, double rfPowerLeveldBm, int port, double *currentFreqDL, double *currentFreqUL, bool autoConvert = false, double rfFreqHz = 0.0);
 ...
 * \return ERR_OK if no errors; otherwise call LP_GetErrorString() for detailed error message.
 */
IQMEASURE_API int       LP_SetVsgPow(double rfPowerLeveldBm);

//! Set VSG Freq 
/*!
 * \param[in] rfFreqHz, The center frequency of VSG (Hz). Set real center frequency
 * \return ERR_OK if no errors; otherwise call LP_GetErrorString() for detailed error message.
 */
IQMEASURE_API int       LP_SetVsgFreq(double rfFreqHz);
 ...
 *				- =4: RF port 3 (RF3)
 *				- =5: RF port 4 (RF4)
 * \param[out] currentFreqUL, the converted frequency value for uplink VSA.
 * \param[out] currentFreqDL, the converted frequency value for downlink VSG if autoConvert is true.
 * \param[in] autoConvert, if enabled, the downlink frequency would converted and set to VSG. 
 * \param[in] rfFreqHz The center frequency of VSA (Hz). Set real center frequency, if set to non-zero value, the frequency value would overwrite the setting for BAND, CHANNEL.
 * \param[in] triggerLevelDb, The trigger level (dBm) used for signal trigger (ignored in Free-run and External Trigger Modes)
 * \param[in] triggerPreTime, The pre-trigger time used for signal capture
 * \param[in] dFreqShiftHz, Frequency Shift
 *
 ...
 * \remark For VSA to work optimally with the input signal, set rfAmplDb to the peak power of the input signal.
 */
IQMEASURE_API int       LP_SetVsaNBIoT(int band, double channel, double rfAmplDb, int port, double *currentFreqUL, double *currentFreqDL, bool autoConvert=false, double rfFreqHz = 0.0, double triggerLevelDb=-25, double triggerPreTime=150e-6, double dFreqShiftHz = 0.0, double triggerGapSec = 6e-6);

//! Set VSA Freq 
/*!
 * \param[in] rfFreqHz, The center frequency of VSA (Hz). Set real center frequency
 * \return ERR_OK if no errors; otherwise call LP_GetErrorString() for detailed error message.
 */
IQMEASURE_API int       LP_SetVsaFreq(double rfFreqHz);
//! Disable VSA/VSG module
/*!
 * \return ERR_OK if no errors; otherwise call LP_GetErrorString() for detailed error message.
 */
IQMEASURE_API int       LP_SetPortOFF();
 ...
 * \return ERR_OK if no errors; otherwise call LP_GetErrorString() for detailed error message.
 */
IQMEASURE_API int       LP_SetVsaRef(double rfAmplDb);

//! Perform NB-IoT Analysis on current capture
/*!
 * \param[in] packetOffsetForAnalysis, Offset in slots into the capture data for the start of the calculation.
 * \param[in] packetCountForAnalysis, Number of slots to calculate. 
 * \param[in] setRBW, Set LTE NB special RBW state for occupied BW calculation enabled.   
 * \return ERR_OK if no errors; otherwise call LP_GetErrorString() for detailed error message.
 */
IQMEASURE_API int		LP_AnalyzeNBIoT(int packetOffsetForAnalysis, int packetCountForAnalysis, bool setRBW = false);
//! Set CELL ID and subcarrier setting for NB-IoT
/*!
 * \param[in] cellId, Sets the Cell ID.
 * \param[in] subCarrier, Sets the narrow band subcarrier number parameter parsing from higher layer(1 to 12). 
 * \param[in] subCarrierAlloc, Sets the narrow band subcarrier allocation parameter parsing from higher layer(12 to 18). 
 * \return ERR_OK if no errors; otherwise call LP_GetErrorString() for detailed error message.
 */
IQMEASURE_API int		LP_SetModeNBIoT( unsigned int cellId, double subCarrierSpacing, unsigned int subCarrier, unsigned int subCarrierAlloc);
//! Sets the narrow band subcarrier spacing in Hz, 3.75 kHz or 15 kHz. 
/*!
 * \param[in] subCarrierSpacing, Narrow Band Subcarrier Spacing(3750 to 15000).
 * \return ERR_OK if no errors; otherwise call LP_GetErrorString() for detailed error message.
 */
IQMEASURE_API int		LP_SetSpacingNBIoT( double subCarrierSpacing);
//**************************************************************************//
//                            NB-IoT END                                    //
//**************************************************************************//
 ...
 * \return ERR_OK if no errors; otherwise call LP_GetErrorString() for detailed error message.
 */
IQMEASURE_API int		LP_SetVsgCATM(int band, double channel, double rfPowerLeveldBm, int port, double *currentFreqDL, double *currentFreqUL, bool autoConvert = false, double rfFreqHz = 0.0, bool isCW=false);
 ...
 * \remark For VSA to work optimally with the input signal, set rfAmplDb to the peak power of the input signal.
 */
IQMEASURE_API int		LP_SetVsaCATM(int band, double channel, double rfAmplDb, int port, double *currentFreqUL, double *currentFreqDL, bool autoConvert=false, double rfFreqHz = 0.0, double triggerLevelDb=-25, double triggerPreTime=150e-6, double dFreqShiftHz = 0.0, double triggerGapSec = 6e-6);
 ...
 * \return ERR_OK if no errors; otherwise call LP_GetErrorString() for detailed error message.
 */
IQMEASURE_API int		LP_AnalyzeCATM(int packetOffsetForAnalysis, int packetCountForAnalysis, int enable_POW, int enable_TXQ, int enable_SPEC);

//! Set CELL ID and subcarrier setting for NB-IoT
/*!
 * \param[in] bandwidth, Sets the channel bandwidth for LTE signals.
 * \param[in] RBOffset, Sets the RB Offset per subframe for each frame. 
 * \param[in] RBDuration, Sets the RB Duration per subframe for each frame. 
 * \return ERR_OK if no errors; otherwise call LP_GetErrorString() for detailed error message.
 */
IQMEASURE_API int		LP_SetModeCATM( double bandwidth, unsigned int RBOffset, unsigned int RBDuration);
//**************************************************************************//
//                            CAT-M END                                    //
//**************************************************************************//
//**************************************************************************//
...
* \return ERR_OK if no errors; otherwise call LP_GetErrorString() for detailed error message.
*/
IQMEASURE_API int LP_AnalyzeGPRFPowerWithOffsetFreq(double intervalLength, double measurementLength, int filterType , int filterBWinhz, double offsetInhz = 0.0);
...
//**************************************************************************//
//                            SequenceBuilder BEGIN                         //
//**************************************************************************//

//! Initializes the sequence builder structure
/*!
 * \param[in] String describing the measurement type.
				Valid types: "CFO"
							"Timing"
							"Power Control"
							"APRX"
 ...
 * \return ERR_OK if no errors; otherwise call LP_GetErrorString() for detailed error message.
*/
IQMEASURE_API int LP_Meas_InitSeqBuilder(char *measurement_type);

//! Sets input parameters for a single sequence step. Called multiple times for a sequence 
//! containing multiple steps
/*!
 * \param[in] SeqBuildParams struct containing all input parameters

 * \return ERR_OK if no errors; otherwise call LP_GetErrorString() for detailed error message.
*/
IQMEASURE_API int LP_Meas_SetSeqinputParams(SeqBuildParams *input);

//! Builds and executes the sequence
/*!
 * \param[in] None

 * \return ERR_OK if no errors; otherwise call LP_GetErrorString() for detailed error message.
*/
IQMEASURE_API int LP_Meas_BuildAndExecute();

//! Checks if sequence executed correctly
/*!
 * \param[in] None

 * \return ERR_OK if no errors; otherwise call LP_GetErrorString() for detailed error message.
*/
IQMEASURE_API int LP_Meas_CheckSeqStatus();
