...
public:	
   ...
   virtual ~iqapiModulationWave(void){} ;	//!< Destructor
   ...
   iqapiModulationWave(const iqapiModulationWave &src) {};
    ...
    };		//!< Saves an \c iqapiModulationWave object to a \c .mod file(set by filename), which can be downloaded to a VSG via this API, or by the VSG panel in the IQsignal for MIMO application.    
